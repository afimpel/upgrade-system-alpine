...
if [ $UUID != 0 ]; then
    ...
    echo "%wheel ALL=(ALL) NOPASSWD: /sbin/poweroff, /sbin/reboot, /sbin/shutdown, /usr/bin/upgrade-syste*, /usr/bin/check-package" >> /etc/sudoers.d/wheel
    adduser $(getent passwd 1000 | cut -d: -f1) wheel

    wget --no-cache -O /usr/bin/check-package ${URLgithub}/main/check-package
    ...
    chmod 777 /usr/bin/check-package -R
